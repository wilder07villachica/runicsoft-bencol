...
export default function ResetPasswordPage() {
  ...
  const submit = async (e) => {
    ...
    try {
      const { data } = await authService.resetPassword({ token: params.get("token"), nuevaPassword })
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white grid place-items-center p-5">
      <div className="max-w-md w-full rounded-3xl border border-white/10 bg-white/[0.04] p-8">
        ...
        <p className="text-slate-400 mb-6">Crea una contraseña segura de mínimo 8 caracteres.</p>
        ...
        <form onSubmit={submit} className="space-y-4">
          <input type="password" value={nuevaPassword} onChange={(e) => setNuevaPassword(e.target.value)} required minLength={8} className="w-full rounded-2xl bg-slate-900 border border-white/10 px-4 py-3 outline-none" />
          <button disabled={loading} className="w-full rounded-2xl bg-cyan-500 hover:bg-cyan-400 text-slate-950 font-bold py-3 disabled:opacity-60">{loading ? "Guardando..." : "Restablecer contraseña"}</button>
        </form>
