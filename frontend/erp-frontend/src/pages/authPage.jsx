  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (mode === "login") {
        ...
        setMessage(
          data.mensaje ||
            "Usuario registrado. Se creó una empresa propia para tu cuenta. Verifica tu correo para iniciar sesión."
        )
      ...
      setError(err.response?.data?.mensaje || err.message || "No se pudo completar la operación.")
    } finally {
      setLoading(false)
    }
  }
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 grid lg:grid-cols-[1.1fr_.9fr]">
      <section className="hidden lg:flex flex-col justify-between p-10 bg-gradient-to-br from-slate-950 via-slate-900 to-cyan-950">
        ...
        <div className="max-w-xl">
          ...
          <h2 className="text-5xl font-black leading-tight mb-5">
            Cada empresa trabaja con sus propios datos.
          </h2>
          <p className="text-slate-300 text-lg">
            Al crear una cuenta, el sistema genera una empresa propia y separa clientes, productos,
            ventas, caja y finanzas.
          </p>
        </div>

        <div className="grid grid-cols-3 gap-4 text-sm text-slate-300">
          <div className="rounded-2xl bg-white/5 p-4 border border-white/10">Empresa propia</div>
          <div className="rounded-2xl bg-white/5 p-4 border border-white/10">JWT seguro</div>
          <div className="rounded-2xl bg-white/5 p-4 border border-white/10">Datos aislados</div>
        </div>
      </section>
      ...
      <section className="flex items-center justify-center p-5 sm:p-8">
        <div className="w-full max-w-md rounded-3xl border border-white/10 bg-white/[0.04] shadow-2xl p-6 sm:p-8">
          <div className="mb-7">
            ...
            <p className="text-slate-400 mt-2">
              {mode === "login"
                ...
                : "Se creará una empresa propia para este usuario."}
            </p>
          </div>

          {message && (
            <div className="mb-4 rounded-xl bg-emerald-500/10 border border-emerald-400/30 text-emerald-200 px-4 py-3 text-sm">
              {message}
            </div>
          )}

          {error && (
            <div className="mb-4 rounded-xl bg-red-500/10 border border-red-400/30 text-red-200 px-4 py-3 text-sm">
              {error}
            </div>
          )}
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <label className="block">
              ...
              {mode === "register" && correoDisponible !== null && (
                <small className={correoDisponible ? "text-emerald-300" : "text-red-300"}>
                  {correoDisponible ? "Correo disponible" : "Correo ya registrado"}
                </small>
              )}
            </label>
            ...
            {mode === "register" && (
              <label className="block">
                ...
                {celularDisponible !== null && (
                  <small className={celularDisponible ? "text-emerald-300" : "text-red-300"}>
                    {celularDisponible ? "Celular disponible" : "Celular ya registrado"}
                  </small>
                )}
              </label>
            )}
            ...
            <button
              disabled={loading}
              className="w-full rounded-2xl bg-cyan-500 hover:bg-cyan-400 text-slate-950 font-bold py-3 transition disabled:opacity-60"
            >
          ...
          <button
            onClick={cambiarModo}
            className="w-full mt-5 text-sm text-slate-300 hover:text-white"
          >
